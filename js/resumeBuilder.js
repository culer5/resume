...
projects.display = function () {
    if (projects.projects.length > 0) {
        for (var k = 0, len = projects.projects.length; k < len; k++) {
            $("#projects").append(HTMLprojectStart);

            var formattedProjectTitle = HTMLprojectTitle.replace("%data%", projects.projects[k].title).replace("#", projects.projects[k].url);
            var formattedProjectDates = HTMLprojectDates.replace("%data%", projects.projects[k].datesWorked);
 ...
 /*            if (projects.projects[p].images[img].length > 0) {
        for (var p = 0, pLen = projects.projects[p].images[img].length; p < pLen; p++)
